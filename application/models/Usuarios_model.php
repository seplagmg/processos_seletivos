...
class Usuarios_model extends CI_Model {
        ...
        public function get_usuarios($id='', $cpf='', $perfil='', $vaga = '', $ativo = false,$instituicao = '',$nome='',$email=''){
                if(strlen($id) > 0){
                ...
                if(strlen($nome) > 0){
                        $this -> db -> where('vc_nome like \'%'.$nome.'%\'');
                }
                if(strlen($email) > 0){
                        $this -> db -> where('vc_email', $email);
                }

                $se_perfil=false;
                if(strlen($vaga) >0){
                        $this -> db -> where("((pr_usuario in (select es_usuario from rl_vagas_avaliadores where es_vaga={$vaga}) and en_perfil = 2))");
                        $se_perfil=true;
                }
                ...
                if($this -> session -> perfil == 'sugesp' || $this -> session -> perfil == 'orgaos'){
                        //validação para o supervisor
                        if(strlen($instituicao) > 0){
                                if(stristr($instituicao,"<>")){
                                        $instituicao=trim(str_replace("<>","",$instituicao));
                                        //alterado para pegar valores nulos
                                        $this -> db -> where('(u.es_instituicao <> '.$instituicao. ' or u.es_instituicao is null)');
                                }
                                else{
                                        $this -> db -> where('u.es_instituicao',$instituicao);
                                }
                        }
                        if($id =='' && $cpf==''){
                                if(!$se_perfil){
                                        $this -> db -> where("(u.en_perfil in ('orgaos','avaliador') or u.pr_usuario = {$this -> session -> uid})");
                                }
                                if($this -> session -> brumadinho == '1' && $this -> session -> pps == '1'){
                                        //seleciona todos os tipos de editais
                                }
                                else if($this -> session -> brumadinho == '1'){
                                        $this -> db -> where('bl_brumadinho','1');
                                }
                                else if($this -> session -> pps == '1'){
                                        $this -> db -> where('bl_pps','1');
                                }
                        }
                }
                else{
        ...
        public function create_usuario($dados){
                ...
                if(!isset($dados['NovoTermo'])){
                        $dados['NovoTermo'] = null;
                }
                if(!isset($dados['instituicao']) || strlen($dados['instituicao']) == 0){
                        $dados['instituicao'] = null;
                }
                if(!isset($dados['brumadinho']) || strlen($dados['brumadinho']) == 0){
                        $dados['brumadinho'] = null;
                }
                if(!isset($dados['pps']) || strlen($dados['pps']) == 0){
                        $dados['pps'] = null;
                }
	...
	public function login($login, $senha){
        ...
        public function log($tipo, $local, $texto, $tabela='', $chave='', $email=true){
                ...
                if($email && ($tipo==1 || $tipo==2)){
                        ...
                        $this -> email -> to($this -> config -> item('administrador'));
                        if($tipo==1){
